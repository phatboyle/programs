...
class Spirograph(model.Background):
    ...
    def doSpirograph(self):
        ...
        if not comp.chkAutoRefresh.checked:
            canvas.autoRefresh = 0

        if comp.chkClearDisplay.checked:
            canvas.clear()
        ...
        if r == 0:
            r = 0.0000001
        exprResult = (rSum * t) / r
        lastX = int(round(rSum*math.cos(t) - O*math.cos(exprResult) + xOffset))
        lastY = int(round(rSum*math.sin(t) - O*math.sin(exprResult) + yOffset))
        self.keepDrawing = 1
        while self.keepDrawing and abs(t) <= revolutions:
            self.statusBar.text = 'Revolutions: %1.2f' % t
            exprResult = (rSum * t) / r
            x = int(round(rSum*math.cos(t) - O*math.cos(exprResult) + xOffset))
            y = int(round(rSum*math.sin(t) - O*math.sin(exprResult) + yOffset))
            if drawLines:
