...
from modules import actionBinderDialog
...
class ResourceEditor(model.Background):
    ...
    def pointInControl(self, position):
        ...
        for name in self.components.order:
            ...
            if not hasattr(control, 'position'):
                continue
            r = control.GetRect()
            #print '    ', control, r
...
##            self.on_mouseUp(event)
    ...
    def create_component(self, desc, offsets, promptString, errString):
        ...
        if result.accepted:
            ...
            self.startName = name
            new_component = self.components[self.startName]
            self.startPosition = getattr(new_component, 'position', (0,0))
            self.startSize = getattr(new_component, 'size', (0,0))
            self.offset = (0, 0)
            if hasattr(new_component, 'position'):
                self.showSizingHandles(name)
            else:
                self.hideSizingHandles()
            self.documentChanged = True
            ...
            if hasattr(new_component, 'position'):
                wx.EVT_LEFT_DOWN(new_component, self.on_mouseDown)
                wx.EVT_LEFT_UP(new_component, self.on_mouseUp)
                wx.EVT_MOTION(new_component, self.on_mouseDrag)
    ...
    def on_editActionBinder_command(self, event):
        actionBinderDialog.runDialog(self)
