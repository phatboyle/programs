...
class PropertyEditor(model.Background, ChangeListener):
    ...
    def updateComponent(self):
        ...
        checkItems = ['enabled', 'visible', 'editable', 'checked', 'default', 'rules', 'labels', 'ticks', 'horizontalScrollbar']
        popItems = ['layout', 'border', 'style', 'alignment', 'stringSelection']
        cantmodify = ['id', 'name', 'alignment', 'layout', 'style', 'border', 'horizontalScrollbar', \
    ...
    def displayProperty(self, wName, wClass, propName):
        ...
        ##widget = self.components[wName]
        widget = self._comp[wName]
        if propName in ['label', 'stringSelection', 'text', 'toolTip'] or propName in checkItems:
