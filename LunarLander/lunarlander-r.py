...
def display_final(out_of_orbit):#this one
    print "display final"
    if  out_of_orbit:
        final1 = "Game over."
        final2 = "You have escaped the moons gravity."
        final4 = "No landing for You today!"
        ...
        final4 = "No landing for you today!"#this one
...
while True:
    ...
    fps = clock.get_fps()
    if fps < 1:  fps = 30
    print y_pos, velocity
    if y_pos <= 0 and velocity >= +100:
        display_final(True)
        pygame.display.flip()
    else:
        if height > 0.01:    
